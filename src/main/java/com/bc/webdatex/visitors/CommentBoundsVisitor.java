...
public class CommentBoundsVisitor extends AbstractNodeVisitor
  implements HasBounds, Serializable
{
  ...
  private void updateComment(Node node)
  {
    ...
    if ((!this.started) && (n != -1))
    {
      Log.getInstance().log(Level.FINER, "Entering Comment: {0}", getClass(), node);
      ...
      Log.getInstance().log(Level.FINER, "Exiting Comment: {0}", getClass(), node);
