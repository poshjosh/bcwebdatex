...
public class DefaultBoundsFilter extends ParentNodeBoundsFilter {
  ...
  protected boolean isDefaultAccept(Node node)
  {
    ...
    if (defaultAccept) {
      Log.getInstance().log(Level.FINER, "DEFAULT Accept: {0}", getClass(), tagName);
    }
  ...
  private boolean isValidContent(Node node)
  {
    ...
    Log.getInstance().log(Level.FINER, "Within title: {0}, body: {1}, Node: {2}", getClass(), Boolean.valueOf(this.withinTitle), Boolean.valueOf(this.withinBody), tag);
